...
const [showSpend, setShowSpend] = useState(true);
  ...
  // Helper to format WhatsApp link dynamically for refunds/support
  const getWhatsAppRefundUrl = (order: Order | null) => {
    if (!order) return "#";
    // Strip special characters and format to international standard (Nigeria '234')
    const adminPhone = "2349000000000"; // Replace with your actual admin contact number if needed
    const message = encodeURIComponent(
      `Hello, I would like to request a refund/assistance for cancelled Order #${order.reference.slice(-6).toUpperCase()} (Tracking: ${order.trackingNumber}).`
    );
  ...
  <div className="md:col-span-2 bg-neutral-50 rounded-3xl p-6 md:p-8">
    <div className="flex items-center justify-between">
      ...
      {/* Hide/Show Toggle Button next to Header */}
      <button
        onClick={() => setShowSpend(!showSpend)}
        className="flex items-center gap-1.5 text-xs text-neutral-400 hover:text-neutral-600 transition-colors py-1 px-2.5 rounded-full hover:bg-neutral-100"
        title={showSpend ? "Hide lifetime spend" : "Show lifetime spend"}
        type="button"
      >
        {showSpend ? (
          <>
    ...
    <div className="mt-6 grid sm:grid-cols-2 gap-4 text-sm">
      ...
      <Info
        ...
        /* Ternary operator hides the amount if showSpend is false */
        value={
          showSpend
            ? formatNaira(userOrders.reduce((s, o) => s + o.total, 0))
            : "₦ ••••••"
        }
      />
    </div>
      ...
      <ConfirmModal
        ...
        whatsappUrl={getWhatsAppRefundUrl(confirmCancel)}
      />
...
<AnimatePresence>
  {issueFor && (
    <motion.div
      ...
      onClick={() => !isSubmitting && setIssueFor(null)} // Prevent closing while submitting
    >
      <motion.div
      ...
      >
        ...
        <textarea
          ...
          className="mt-4 w-full border border-black/15 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-black disabled:bg-neutral-100 disabled:text-neutral-400"
        />
        <div className="mt-5 flex gap-3 justify-end">
          <button
            onClick={() => {
              setIssueText(""); // Clear text on cancel
              setIssueFor(null);
            }}
            disabled={isSubmitting}
            className="border border-black/15 rounded-full px-5 py-2.5 text-sm disabled:opacity-50"
          >
            Close
          </button>
          <button
            onClick={async () => {
              if (issueFor && issueText.trim() && !isSubmitting) {
                setIsSubmitting(true);
                try {
                  // Wait for the async server action to complete
                  await reportIssue(issueFor.id, issueText.trim());
                  
                  // SUCCESS FLOW: Clear the text, reset loading state, and close modal
                  setIssueText(""); 
                  setIssueFor(null);
                } catch (error) {
                  // ERROR FLOW: Don't close the modal so the customer sees it failed!
                  console.error("Submission failed:", error);
                  alert("Could not report issue. Please check your connection or try again.");
                } finally {
            ...
            className="bg-black text-white rounded-full px-5 py-2.5 text-sm flex items-center gap-2 disabled:bg-neutral-400"
          >
          ...
          >
            ...
            <div className="mt-5 flex flex-col gap-2">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-center text-xs text-neutral-500 hover:text-black underline mt-2"
              >
                Need manual help? Chat on WhatsApp
              </a>
            </div>
          </motion.div>
        </motion.div>
